...
- **Bug Fix**: Resolved an infinite loading issue in `CoastalAvailability` and migrated to Next.js v16 (including `middleware` -> `proxy` transition) to fix `ChunkLoadError`.
